...
# run simulation on HPC
# one SLURM array tasks corresponds to one simulation run, from the same scenario!

################################################################################
################################################################################

# configuration from environment with local defaults
...
stopifnot(all(diff(emis$mu1) > 0)) # mu1 ascending
R_vec <- rep(30, J)
...
set.seed(seed)
# set to all (all 100 simulation runs are cv_aic runs)
...
.s <- .Random.seed
for (r in seq_len(sim_runs)) { run_seeds[[r]] <- .s; .s <- parallel::nextRNGStream(.s) }
...
# penalty matrix (2-order differences)
...
# global initial starting parameters
...
# function to fit models on the HPC
...
run_one <- function(i) {
  ...
  # NB warm start based on best HMM
  t_nb <- system.time({
    if (!is.null(hmm_pars)) {
  ...
  t_nonp <- system.time({
    ...
    # for convergence safeguard
    safeguard_ran <- FALSE  
    safeguard_swapped <- FALSE   
    LAMBDA_CAP <- 1e6 # might indicate problem, so just rerun those runs with different init lambda
    # then choose the result with the higher likelihood (completely self-contained)
    LAMBDA_LO <- 1e-3
    ...
    if (!is.null(nonp_pars)) {
      ...
      if (any(hit_hi) || any(hit_lo)) {
        safeguard_ran <- TRUE
        init_lam <- lam  # keep well-behaved states where they landed
        # try new initial values for suspicious states
        init_lam[hit_hi] <- 0.1 
        init_lam[hit_lo] <- 1e4 
  ...
  t_cv_nonparametric <- system.time({
    if (do_cv_aic) {
      cv_lambda <- safe(greedy_lambda_cv(data_list, J, R_vec, S, par_0,
                                         exp_grid = 0:6, start_exp = rep(2, J), K = 5))
